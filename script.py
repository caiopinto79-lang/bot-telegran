...
<body>
    ...
    <script>
        function verificarIdade(event) {
            event.preventDefault(); // Impede o link de abrir direto sem o alerta
            const maior = confirm("Atenção: Este site contém material adulto (+18).\n\nVocê confirma que tem 18 anos ou mais e deseja continuar?");
            if (maior) {
                window.location.href = "/conteudos"; // Redireciona na mesma aba para a página de conteúdos
            }
        }
    </script>
</body>
...
# PÁGINA 2: Conteúdos Exclusivos com a sequência solicitada (Privacy, Prévias e VIP)
HTML_CONTEUDOS = """
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body>
    <div class="container">
        ...
        <!-- Sequência solicitada: Privacy, Canal de Prévias, Grupo VIP -->
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port)ciais do Mercado Pago configuradas via variáveis de ambiente no Render
MP_ACCESS_TOKEN = os.environ.get("MP_ACCESS_TOKEN", "SEU_ACCESS_TOKEN_DO_MERCADO_PAGO")
sdk = mercadopago.SDK(MP_ACCESS_TOKEN)

# PÁGINA 1: Início (Redes Sociais + Botão +18 com Alerta de Maioridade)
HTML_INDEX = """
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Yasmin - Links Oficiais</title>
    <style>
        :root {
            --bg-color: #0f172a;
            --card-bg: #1e293b;
...
# PÁGINA 2: Conteúdos Exclusivos (+18, Prévias, Privacy e VIP)
HTML_CONTEUDOS = """
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Yasmin - Conteúdos Exclusivos</title>
    <style>
        :root {
            --bg-color: #0f172a;
            --card-bg: #1e293b;
...
def criar_pagamento():
    try:
        ...
        payment_data = {
            "transaction_amount": float(dados_cliente.get("valor", 29.90)),
            "description": "Acesso Canal VIP Telegram - Yasmin",
            "payment_method_id": "pix",
            "payer": {
                "email": dados_cliente.get("email", "cliente@email.com"),
                "first_name": dados_cliente.get("nome", "Cliente")
